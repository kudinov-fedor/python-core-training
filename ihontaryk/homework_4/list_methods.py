...
def extend_user_details():
    users = RandomUser.generate_users(1, {'nat': 'us'})

    user_details = [users[0].get_username(), users[0].get_last_name(), users[0].get_first_name(), users[0].get_gender()]
    user_contacts = [users[0].get_zipcode(), users[0].get_state(), users[0].get_city(), users[0].get_street(), users[0].get_phone()]
    user_details.extend(user_contacts)

    return user_details


if __name__ == '__main__':
    print(extend_user_details())
