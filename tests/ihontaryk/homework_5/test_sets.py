...
@pytest.mark.parametrize('set1, number1, expected_resul',
                         [({'a', 'b'}, 1, set()),
                          ({'a', 'b', 'c'}, 1, {'b'}),
                          ({1, 2, 3, 4, 5, 6, 7}, 3, {4}),
                          ({'a', 'b', 'abc', 'abc2'}, 2, set()),
...
def test_remove_min_max_loop_positive(set1, number1, expected_resul):
    """
    verify positive cases for remove_min_max_loop function
    """
    assert remove_min_max_loop(set1, number1) == expected_resul
